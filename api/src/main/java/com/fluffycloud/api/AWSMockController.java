...
public class AWSMockController
{
	...
	public String mockAWSCloud(HttpServletRequest request)
	{
		...
		/*TODO
		 * Request have the params present in command For eg: CLI command: aws
		 * ec2 describe-images --image-ids ami-xxxxx --endpoint-url
		 * http://127.0.1.1:8080/aws
		 * 
		 * Params in request are:
		 *  ImageId.1 : ami-xxxxx
		 *  Action : DescribeImages
		 *  SignatureMethod : HmacSHA256 
		 *  AWSAccessKeyId : AKIAIDGQBXU3XTSPTPOA 
		 *  SignatureVersion : 2 
		 *  Version : 2013-10-15
		 *  Signature : olDIF1AjB06H6E7SlThtJC/6ojhWgBUtOmyUGZD+sqo= 
		 *  Timestamp : 2014-09-10T10:26:20.985215
		 * 
		 * 
		...
		/*
		 * TODO 
		 * 1. Need to set content type for the response. 
		 * 
		 * 2. Using CLI use
		 * debug mode with any command then it will show the response XML which
		 * is returned by AWS APIs. 
		 * 
		 * 3. When JSON returned in response with
		 ...
		 * 
		 *  then the CLI client shows: Not well-formed(invalid token): line 1, column 0
		 *  
		 *  4. Issue is to create the XML response which can be parsed by CLI client. 		
		 * 
		 */
